...
// ** Actions Imports
import { updateAdmin, fetchPermissions } from 'src/store/apps/admin'
...
const SidebarEditAdmin = props => {
  // ** Props
  const { open, toggle, adminData } = props
  ...
  // Set form values when adminData changes
  useEffect(() => {
    if (adminData && open) {
      setValue('name', adminData.name || '')
      setValue('phone', adminData.phone || '')
      
      // Set permissions - convert permission objects to IDs for the form
      if (adminData.permissions && Array.isArray(adminData.permissions)) {
        const activePermissionIds = adminData.permissions
          .filter(permission => permission.have === 1)
          .map(permission => permission.id)
        
        setValue('permissions', activePermissionIds)
      } else {
        setValue('permissions', [])
      }
    }
  }, [adminData, open, setValue])
  ...
  const onSubmit = data => {
    ...
    console.log('Form data being submitted for update:', data)
    
    dispatch(updateAdmin({formData:formData,id:adminData.id }))
      .then(() => {
      ...
      .catch(error => {
        toast.error('Error updating admin: ' + (error.message || 'Unknown error'), {
          position: 'bottom-right'
        })
      })
  }
...
export default SidebarEditAdmin